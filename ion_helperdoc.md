# ION项目指南
这是一个ION项目指南，你可以用它创建你自己的ion.py 
在这之前，请确保你从<a>https://github.com/wuzhixiangc/ION</a>获取ion_helper.py和ion.py与依赖项
...
### ion.py
就是原本的ion.py文件，ion.py会在导入你的文件并执行init_ion_handle函数和IONMixin类

### `__init_ion_other__.py` 确保它有init_ion_handle函数和IONMixin类
你可以用AsyncTaskPoolMixin让你的类自动有强大的并发能力
如：
```python
from ion_helper import IndexingHandlerGenerator, BaseMixin, IndexableMixin, SearchableMixin, AsyncTaskPoolMixin
...
hash_index_methods = {
    "create": ["create_item", "batch_create"],  # 创建操作：这些方法调用时添加索引
    "update": ["update_item"],                  # 更新操作：这些方法调用时先删后加索引
    "delete": ["delete_item"],                  # 删除操作：这些方法调用时删除索引
    "batch": ["batch_create"]                   # 批量操作：批量创建时批量添加索引
}
# 生成JSON索引处理函数
init_ion_handle = IndexingHandlerGenerator.generate_custom_handler(
    index_type="JSON", #名称
    handler_method_name="smart_{}_index_{}",  # 模板：{}会被替换为index_type和操作类型
    index_methods=hash_index_methods
)
...
{
    "path":"~/ion", # ion依赖的路径
    "ion_proj_path":"你的项目路径",
    "version":"版本"
}
``` 

### 后期`ion_helper.py`，如果没下载到就用这个
```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
